...
const Project = ({ position, title, onClick }) => {
  ...
  const { scale, color, emissive } = useSpring({
      scale: isHovered ? [1.1, 1.1, 1.1] : [1, 1, 1], // Slightly larger scale for a more visible pulse
      color: isHovered ? '#ffffff' : '#aaaaaa',
      emissive: isHovered ? '#88ccff' : '#222222',
      config: {
          duration: 200, // Slower duration for the pulse
      }
  });
  ...
  useFrame(() => {
      if (isHovered) {
          ...
          scale.start({ 
              to: [
                  1 + 0.1 * Math.sin(time * 10), // Slower frequency and larger amplitude
  ...
  return (
      <a.mesh
          ...
          <Box args={[50, 50, 50]}>
              <a.meshStandardMaterial attach="material" color={color} emissive={emissive} roughness={0.5} metalness={0.1} transparent opacity={0.6} />
          </Box>
          <Html scaleFactor={10}>
...
const HomeHero = () => {
  ...
  const handleProjectClick = (title) => {
      setSelectedProject(title);  // This changes the state to the selected project title
  };
